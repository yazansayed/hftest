...
class TextSnakePostprocessor(BasePostprocessor):
    ...
    def __init__(self,
                 ...
                 min_center_region_confidence=0.3,
                 min_center_area=30,
    ...
    def __call__(self, preds):
        ...
        # np.save('pred_text_mask.npy',pred_text_mask)
        pred_center_score = preds[1] * pred_text_score
        # np.save('pred_center_score.npy',pred_center_score)
        pred_center_mask = \
            pred_center_score > self.min_center_region_confidence
        tcl_mask = pred_text_mask*pred_center_mask
        # np.save('pred_center_mask.npy',pred_center_mask)
        pred_sin = preds[2]
        pred_cos = preds[3]
        pred_radius = preds[4]
        ...
        # np.save('pred_sin.npy',pred_sin)
        # np.save('pred_cos.npy',pred_cos)

        tcl_mask = fill_hole(tcl_mask).astype(np.uint8)
        # np.save('pred_center_mask.npy',pred_center_mask)

        center_contours, _ = cv2.findContours(tcl_mask, cv2.RETR_TREE,
